const mongoose = require('mongoose');
const clearDB = require('./graphql/clearDB');
const personRatingProducersServices = require('../src/graphql/services/personRatingProducers.services');
const usersServices = require('../src/graphql/services/users.services');
const salespointsServices = require('../src/graphql/services/salespoints.services');
const producersServices = require('../src/graphql/services/producers.services');
const productsServices = require('../src/graphql/services/products.services');
const productTypesServices = require('../src/graphql/services/productTypes.services');
const productTypeCategoriesServices = require('../src/graphql/services/productTypeCategories.services');
const postsServices = require('../src/graphql/services/posts.services');

let tabRatings;

const generateId = () => mongoose.Types.ObjectId();

const populateDB = async() => {
  await clearDB();

  const tabPromises = [];

  // ------------------------------------------------------------- ajout des productTypeCategory
  // --------------------------------------------------------------
  const categoriesAutres = {
    _id: generateId(),
    name: 'Autres',
    image: 'data:image/png;base64,'
  };

  tabPromises.push(productTypeCategoriesServices.addProductTypeCategory(categoriesAutres));

  const categoriesBoissons = {
    _id: generateId(),
    name: 'Boissons',
    image: 'data:image/png;base64,'
  };
  tabPromises.push(productTypeCategoriesServices.addProductTypeCategory(categoriesBoissons));

  const categoriesCereales = {
    _id: generateId(),
    name: 'Céréales',
    image: 'data:image/png;base64,'
  };
  tabPromises.push(productTypeCategoriesServices.addProductTypeCategory(categoriesCereales));

  const categoriesFruits = {
    _id: generateId(),
    name: 'Fruits',
    image: 'data:image/png;base64,'
  };
  tabPromises.push(productTypeCategoriesServices.addProductTypeCategory(categoriesFruits));

  const categoriesLegumes = {
    _id: generateId(),
    name: 'Légumes',
    image: 'data:image/png;base64,'
  };
  tabPromises.push(productTypeCategoriesServices.addProductTypeCategory(categoriesLegumes));

  const categoriesViandes = {
    _id: generateId(),
    name: 'Viandes',
    image: 'data:image/png;base64,'
  };

  tabPromises.push(productTypeCategoriesServices.addProductTypeCategory(categoriesViandes));
  await Promise.all(tabPromises);

  // ---------------------------------------------------------------- ajout des productType ----------------------------------------------------------------

  // ---------------------------------------------------------------- productType: autres ------------------------------------------------------------------
  const productTypeBoisDeFeu = productTypesServices.addProductType(
    {
      _id: generateId(),
      name: 'Bois de Feu',
      image: 'data:image/png;base64,',
      categoryId: categoriesAutres._id
    }
  );
  tabPromises.push(productTypeBoisDeFeu);

  const productTypeFleurs = {
    _id: generateId(),
    name: 'Fleurs',
    image: 'data:image/png;base64,',
    categoryId: categoriesAutres._id
  };
  tabPromises.push(productTypesServices.addProductType(productTypeFleurs));

  const productTypeFromagesProduitsLaitiers = {
    _id: generateId(),
    name: 'Fromages / Produits laitiers',
    image: 'data:image/png;base64,',
    categoryId: categoriesAutres._id
  };
  tabPromises.push(productTypesServices.addProductType(productTypeFromagesProduitsLaitiers));

  const productTypeHuiles = {
    _id: generateId(),
    name: 'Huiles',
    image: 'data:image/png;base64,',
    categoryId: categoriesAutres._id
  };
  tabPromises.push(productTypesServices.addProductType(productTypeHuiles));

  const productTypeMiel = {
    _id: generateId(),
    name: 'Miel',
    image: 'data:image/png;base64,',
    categoryId: categoriesAutres._id
  };
  tabPromises.push(productTypesServices.addProductType(productTypeMiel));

  const productTypeOeufs = {
    _id: generateId(),
    name: 'Oeufs',
    image: 'data:image/png;base64,',
    categoryId: categoriesAutres._id
  };
  tabPromises.push(productTypesServices.addProductType(productTypeOeufs));

  const productTypePains = {
    _id: generateId(),
    name: 'Pain',
    image: 'data:image/png;base64,',
    categoryId: categoriesAutres._id
  };
  tabPromises.push(productTypesServices.addProductType(productTypePains));

  const productTypePatisserie = {
    _id: generateId(),
    name: 'Patisserie',
    image: 'data:image/png;base64,',
    categoryId: categoriesAutres._id
  };
  tabPromises.push(productTypesServices.addProductType(productTypePatisserie));

  const productTypeSapin = {
    _id: generateId(),
    name: 'Sapin',
    image: 'data:image/png;base64,',
    categoryId: categoriesAutres._id
  };
  tabPromises.push(productTypesServices.addProductType(productTypeSapin));

  // ---------------------------------------------------------------- productType: boissons ------------------------------------------------------------------
  const productTypeAlcoolsForts = {
    _id: generateId(),
    name: 'Alcools forts',
    image: 'data:image/png;base64,',
    categoryId: categoriesBoissons._id
  };
  tabPromises.push(productTypesServices.addProductType(productTypeAlcoolsForts));

  const productTypeBiereCidre = {
    _id: generateId(),
    name: 'Bières / Cidres',
    image: 'data:image/png;base64,',
    categoryId: categoriesBoissons._id
  };
  tabPromises.push(productTypesServices.addProductType(productTypeBiereCidre));

  const productTypeJusDeFruits = {
    _id: generateId(),
    name: 'Jus de fruits',
    image: 'data:image/png;base64,',
    categoryId: categoriesBoissons._id
  };
  tabPromises.push(productTypesServices.addProductType(productTypeJusDeFruits));

  const productTypeLait = {
    _id: generateId(),
    name: 'Lait',
    image: 'data:image/png;base64,',
    categoryId: categoriesBoissons._id
  };
  tabPromises.push(productTypesServices.addProductType(productTypeLait));

  const productTypeMousseux = {
    _id: generateId(),
    name: 'Mousseux',
    image: 'data:image/png;base64,',
    categoryId: categoriesBoissons._id
  };
  tabPromises.push(productTypesServices.addProductType(productTypeMousseux));

  const productTypeVin = {
    _id: generateId(),
    name: 'Vin',
    image: 'data:image/png;base64,',
    categoryId: categoriesBoissons._id
  };
  tabPromises.push(productTypesServices.addProductType(productTypeVin));

  // ---------------------------------------------------------------- productType: céréales ------------------------------------------------------------------
  const productTypeFarine = {
    _id: generateId(),
    name: 'Farine',
    image: 'data:image/png;base64,',
    categoryId: categoriesCereales._id
  };
  tabPromises.push(productTypesServices.addProductType(productTypeFarine));

  const productTypePolenta = {
    _id: generateId(),
    name: 'Polenta',
    image: 'data:image/png;base64,',
    categoryId: categoriesCereales._id
  };
  tabPromises.push(productTypesServices.addProductType(productTypePolenta));

  const productTypeQuinoa = {
    _id: generateId(),
    name: 'Quinoa',
    image: 'data:image/png;base64,',
    categoryId: categoriesCereales._id
  };
  tabPromises.push(productTypesServices.addProductType(productTypeQuinoa));

  const productTypePates = {
    _id: generateId(),
    name: 'Pâtes',
    image: 'data:image/png;base64,',
    categoryId: categoriesCereales._id
  };
  tabPromises.push(productTypesServices.addProductType(productTypePates));

  const productTypePomme = {
    _id: generateId(),
    name: 'Fruits',
    image: 'data:image/png;base64,',
    categoryId: categoriesFruits._id
  };
  tabPromises.push(productTypesServices.addProductType(productTypePomme));

  const productTypePoire = {
    _id: generateId(),
    name: 'Légumes',
    image: 'data:image/png;base64,',
    categoryId: categoriesFruits._id
  };
  tabPromises.push(productTypesServices.addProductType(productTypePoire));

  await Promise.all(tabPromises);

  // FIXME: ajouter les catégories manquantes...!

  // ------------------------------------------------------------------ ajout de 4 producers
  // -------------------------------------------------------------------

  // on ajoute un producteur
  const producer1 = await producersServices.addProducer(
    {
      _id: generateId(),
      firstname: 'Antoine',
      lastname: 'Rochaille',
      email: 'antoine@paysan.ch',
      password: '1234abcd',
      image: 'Ceci est une image encodée en base64!',
      phoneNumber: '0761435196',
      description: 'description',
      website: 'foodlocal.ch'
    }
  );

  // on valide ce producteur
  tabPromises.push(producersServices.validateAProducer(producer1.id, true));

  // on ajoute des produits au producteur
  tabPromises.push(productsServices.addProduct({
    _id: generateId(),
    description: 'Une tomme monnnnstre bonne!',
    productTypeId: productTypeFromagesProduitsLaitiers._id
  }, producer1.id));
  tabPromises.push(productsServices.addProduct({
    _id: generateId(),
    description: 'Du lait hyper frais!',
    productTypeId: productTypeLait._id
  }, producer1.id));
  tabPromises.push(productsServices.addProduct({
    _id: generateId(),
    description: 'Des spaghettis fait maison!',
    productTypeId: productTypePates._id
  }, producer1.id));
  tabPromises.push(productsServices.addProduct({
    _id: generateId(),
    description: 'Une biere de ouf!',
    productTypeId: productTypeBiereCidre._id
  }, producer1.id));
  tabPromises.push(productsServices.addProduct({
    _id: generateId(),
    description: 'Un jus d\'orange trop bon!',
    productTypeId: productTypeJusDeFruits._id
  }, producer1.id));
  tabPromises.push(productsServices.addProduct({
    _id: generateId(),
    description: 'Un jus de pomme trop bon!',
    productTypeId: productTypeJusDeFruits._id
  }, producer1.id));
  tabPromises.push(productsServices.addProduct({
    _id: generateId(),
    description: 'Une polenta faite maison!',
    productTypeId: productTypePolenta._id
  }, producer1.id));

  // on ajoute un point de vente au producteur
  tabPromises.push(producersServices.addSalespointToProducer(producer1.id, {
    name: 'Chez moi',
    address: {
      number: 6,
      street: 'Chemin de par ici',
      city: 'Yverdon',
      postalCode: '1400',
      state: 'Vaud',
      country: 'Suisse',
      longitude: 6.6562137,
      latitude: 46.7702474
    },
    schedule: {
      monday: [
        {
          openingHour: '08:00',
          closingHour: '12:00'
        },
        {
          openingHour: '13:00',
          closingHour: '18:00'
        }
      ],
      tuesday: [],
      wednesday: [
        {
          openingHour: '08:00',
          closingHour: '12:00'
        }
      ],
      thursday: [],
      friday: [
        {
          openingHour: '08:00',
          closingHour: '12:00'
        }
      ],
      saturday: [],
      sunday: []
    }
  }));

  // on ajoute un producteur
  const producer2 = await producersServices.addProducer(
    {
      _id: generateId(),
      firstname: 'Benoît',
      lastname: 'Schöpfli',
      email: 'benoit@paysan.ch',
      password: '1234abcd',
      image: 'Ceci est une image encodée en base64!',
      phoneNumber: '0761435196',
      description: 'description',
      website: 'foodlocal.ch'
    }
  );
    // on valide ce producteur
  tabPromises.push(producersServices.validateAProducer(producer2.id, true));
  // on ajoute des produits au producteur
  tabPromises.push(productsServices.addProduct({
    _id: generateId(),
    description: 'Une tomme monnnnstre bonne!',
    productTypeId: productTypeFromagesProduitsLaitiers._id
  }, producer2.id));
  tabPromises.push(productsServices.addProduct({
    _id: generateId(),
    description: 'Du lait hyper frais!',
    productTypeId: productTypeLait._id
  }, producer2.id));
  tabPromises.push(productsServices.addProduct({
    _id: generateId(),
    description: 'Des spaghettis fait maison!',
    productTypeId: productTypePates._id
  }, producer2.id));
  tabPromises.push(productsServices.addProduct({
    _id: generateId(),
    description: 'Une polenta faite maison!',
    productTypeId: productTypePolenta._id
  }, producer2.id));

  // on ajoute un point de vente au producteur
  tabPromises.push(producersServices.addSalespointToProducer(producer2.id, {
    name: 'Chez moi',
    address: {
      number: 12,
      street: 'Chemin de par là-bas',
      city: 'Yverdon',
      postalCode: '1400',
      state: 'Vaud',
      country: 'Suisse',
      longitude: 6.6772137,
      latitude: 46.7592474
    },
    schedule: {
      monday: [
        {
          openingHour: '08:00',
          closingHour: '12:00'
        },
        {
          openingHour: '13:00',
          closingHour: '18:00'
        }
      ],
      tuesday: [],
      wednesday: [
        {
          openingHour: '08:00',
          closingHour: '12:00'
        }
      ],
      thursday: [],
      friday: [
        {
          openingHour: '08:00',
          closingHour: '12:00'
        }
      ],
      saturday: [],
      sunday: []
    }
  }));

  // on ajoute un producteur
  const producer3 = await producersServices.addProducer(
    {
      _id: generateId(),
      firstname: 'James',
      lastname: 'submith',
      email: 'james@paysan.ch',
      password: '1234abcd',
      image: 'Ceci est une image encodée en base64!',
      phoneNumber: '0761435196',
      description: 'description',
      website: 'foodlocal.ch'
    }
  );

  tabPromises.push(producersServices.validateAProducer(producer3.id, true));

  // on ajoute des produits au producteur
  tabPromises.push(productsServices.addProduct({
    _id: generateId(),
    description: 'Une tomme monnnnstre bonne!',
    productTypeId: productTypeFromagesProduitsLaitiers._id
  }, producer3.id));
  tabPromises.push(productsServices.addProduct({
    _id: generateId(),
    description: 'Un jus de pomme trop bon!',
    productTypeId: productTypeJusDeFruits._id
  }, producer3.id));

  // on ajoute un point de vente au producteur
  tabPromises.push(producersServices.addSalespointToProducer(producer3.id, {
    name: 'Chez moi',
    address: {
      number: 6,
      street: 'Chemin d\'ici',
      city: 'Yverdon',
      postalCode: '1400',
      state: 'Vaud',
      country: 'Suisse',
      longitude: 6.6572137,
      latitude: 46.7792474
    },
    schedule: {
      monday: [
        {
          openingHour: '08:00',
          closingHour: '12:00'
        },
        {
          openingHour: '13:00',
          closingHour: '18:00'
        }
      ],
      tuesday: [],
      wednesday: [
        {
          openingHour: '08:00',
          closingHour: '12:00'
        }
      ],
      thursday: [],
      friday: [
        {
          openingHour: '08:00',
          closingHour: '12:00'
        }
      ],
      saturday: [],
      sunday: []
    }
  }));

  // on ajoute un producteur
  const producer4 = await producersServices.addProducer(
    {
      _id: generateId(),
      firstname: 'Jérémie',
      lastname: 'Chaton',
      email: 'jerem@paysan.ch',
      password: '1234abcd'
    }
  );
  tabPromises.push(producersServices.validateAProducer(producer4.id, true));

  // on ajoute un producteur
  const producer5 = await producersServices.addProducer(
    {
      _id: generateId(),
      firstname: 'Monsieur',
      lastname: 'UnValidated',
      email: 'lintrus@paysan.ch',
      password: '1234abcd'
    }
  );

  await Promise.all(tabPromises);

  // ------------------------------------------------------------------- ajout de 2 users
  // ---------------------------------------------------------------------- on ajoute un utilisateur
  const user1 = await usersServices.addUser(
    {
      _id: generateId(),
      firstname: 'Jérémie',
      lastname: 'Chaton',
      email: 'jerem@user.ch',
      password: '1234abcd',
      image: 'ceci est une image encodée en base64!'
    }
  );

  // on ajoute un utilisateur
  const user2 = await usersServices.addUser(
    {
      _id: generateId(),
      firstname: 'Loyse',
      lastname: 'Hug',
      email: 'loyse@user.ch',
      password: '1234abcd'
    }
  );

  // ------------------------------------------------------------------- ajout de ratings
  // ---------------------------------------------------------------------- ajout de ratings pour le producer1
  const rating1p1 = personRatingProducersServices.addOrUpdatePersonRatingProducer({ personId: user1.id, producerId: producer1.id, rating: 5 });
  const rating2p1 = personRatingProducersServices.addOrUpdatePersonRatingProducer({ personId: user2.id, producerId: producer1.id, rating: 3 });
  const rating3p1 = personRatingProducersServices.addOrUpdatePersonRatingProducer({ personId: producer2.id, producerId: producer1.id, rating: 2 });

  // ajout de ratings pour le producer2
  const rating1p2 = personRatingProducersServices.addOrUpdatePersonRatingProducer({ personId: user1.id, producerId: producer2.id, rating: 2 });
  const rating2p2 = personRatingProducersServices.addOrUpdatePersonRatingProducer({ personId: user2.id, producerId: producer2.id, rating: 3 });

  // ajout de ratings pour le producer3
  const rating1p3 = personRatingProducersServices.addOrUpdatePersonRatingProducer({ personId: user1.id, producerId: producer3.id, rating: 5 });
  const rating2p3 = personRatingProducersServices.addOrUpdatePersonRatingProducer({ personId: user2.id, producerId: producer3.id, rating: 4 });
  const rating3p3 = personRatingProducersServices.addOrUpdatePersonRatingProducer({ personId: producer1.id, producerId: producer3.id, rating: 4 });
  const rating4p3 = personRatingProducersServices.addOrUpdatePersonRatingProducer({ personId: producer2.id, producerId: producer3.id, rating: 4 });

  tabPromises.push(rating1p1, rating1p2, rating1p3, rating2p1, rating2p2, rating2p3, rating3p1, rating3p3, rating4p3);
  // -------------------------------------------------------------------- ajout de followers
  // ------------------------------------------------------------------- ajout de 3 followers de producer1
  const follower1p1 = producersServices.addFollowerToProducer(producer1.id, user1.id);
  const follower2p1 = producersServices.addFollowerToProducer(producer1.id, user2.id);
  const follower3p1 = producersServices.addFollowerToProducer(producer1.id, producer2.id);

  // ajout de 2 followers de producer4
  const follower1p2 = producersServices.addFollowerToProducer(producer4.id, producer1.id);
  const follower2p2 = producersServices.addFollowerToProducer(producer4.id, producer2.id);

  tabPromises.push(follower1p1, follower2p1, follower3p1, follower1p2, follower2p2);

  // -------------------------------------------------------------------- ajout de posts -------------------------------------------------------------------
  const post1p1 = postsServices.addPostOfProducer({
    _id: generateId(),
    producerId: producer1.id,
    text: 'Ceci est un 1er post! :D',
    address: {
      number: 6,
      street: 'Chemin de par ici',
      city: 'Yverdon',
      postalCode: '1400',
      state: 'Vaud',
      country: 'Suisse',
      longitude: 6.0562137,
      latitude: 46.3702474
    }
  });

  const post2p1 = postsServices.addPostOfProducer({
    _id: generateId(),
    producerId: producer1.id,
    text: 'Ceci est un 2ème post! :D'
  });
  const post3p1 = postsServices.addPostOfProducer({
    _id: generateId(),
    producerId: producer1.id,
    text: 'Ceci est un 3ème post! :D',
    address: {
      number: 6,
      street: 'Chemin de par ici',
      city: 'Yverdon',
      postalCode: '1400',
      state: 'Vaud',
      country: 'Suisse',
      longitude: 6.6062137,
      latitude: 46.7002474
    }
  });
  const post4p1 = postsServices.addPostOfProducer({
    _id: generateId(),
    producerId: producer1.id,
    text: 'Ceci est un 4ème post! :D',
    address: {
      number: 6,
      street: 'Chemin de par ici',
      city: 'Yverdon',
      postalCode: '1400',
      state: 'Vaud',
      country: 'Suisse',
      longitude: 6.2562137,
      latitude: 46.9702474
    }
  });
  const post5p1 = postsServices.addPostOfProducer({
    _id: generateId(),
    producerId: producer1.id,
    text: 'Ceci est un 5ème post! :D'
  });
  const post6p1 = postsServices.addPostOfProducer({
    _id: generateId(),
    producerId: producer1.id,
    text: 'Ceci est un 6ème post! :D',
    address: {
      number: 6,
      street: 'Chemin de par ici',
      city: 'Yverdon',
      postalCode: '1400',
      state: 'Vaud',
      country: 'Suisse',
      longitude: 6.7562137,
      latitude: 46.6702474
    }
  });

  const post1p2 = postsServices.addPostOfProducer({
    _id: generateId(),
    producerId: producer2.id,
    text: 'Ceci est un 1er post! :D',
    address: {
      number: 6,
      street: 'Chemin de par ici',
      city: 'Yverdon',
      postalCode: '1400',
      state: 'Vaud',
      country: 'Suisse',
      longitude: 6.6362137,
      latitude: 46.7002474
    }
  });
  const post2p2 = postsServices.addPostOfProducer({
    _id: generateId(),
    producerId: producer2.id,
    text: 'Ceci est 2ème post ! :D'
  });

  const post1p3 = postsServices.addPostOfProducer({
    _id: generateId(),
    producerId: producer3.id,
    text: 'Ceci est un post d\'un troisième producteur sans localisation! :D'
  });

  tabPromises.push(post1p1, post2p1, post3p1, post4p1, post5p1, post6p1, post1p2, post2p2, post1p3);

  await Promise.all(tabPromises);
  // ------------------------------------------------------------------------- tableaux
  // ------------------------------------------------------------------------ on regroupe chaque élément dans des tableaux pour les tests d'intégration
  /*
    tabProductTypes = await productTypesServices.getProductTypes();

    tabProductTypeCategories = await productTypeCategoriesServices.getProductTypeCategories();

    tabProducers = await producersServices.getProducers(true);
    tabSalespoints = await salespointsServices.getSalespoints();

    tabUsers = await usersServices.getUsers();
    */

  tabRatings = [rating1p1, rating2p1, rating3p1, rating1p2, rating2p2, rating1p3, rating2p3, rating3p3, rating4p3];
};
it('should populate the database!', populateDB);

module.exports = {
  populateDB,
  getTabProductTypeCategories: () => productTypeCategoriesServices.getProductTypeCategories(),
  getTabProductTypes: () => productTypesServices.getProductTypes(),
  getTabProducers: () => producersServices.getProducers(true),
  getTabSalespoints: () => salespointsServices.getSalespoints(),
  getTabUsers: () => usersServices.getUsers(),
  getTabRatings: () => tabRatings
};
