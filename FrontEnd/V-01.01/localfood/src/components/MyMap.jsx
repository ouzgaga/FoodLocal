import React, { Component } from 'react';
import L from 'leaflet';
import {
  Map, TileLayer, Marker, Popup, CircleMarker,
} from 'react-leaflet';

const myIcon = L.icon({
  iconUrl: 'data:image/png;base64,',
  iconSize: [25, 41],
  iconAnchor: [12.5, 41],
  PopupAnchor: [0, 90],
});

class MyMap extends Component {
  constructor(props) {
    super(props);

    this.state = {

      location: {
        // par défaut, position de Lausanne
        latitude: 46.5333,
        longitude: 6.6667,
      },
      userHasALocation: false, // indique si l'utilisateur a accepté de donner sa position
    };
  }

  componentDidMount() {
    navigator.geolocation.getCurrentPosition((position) => {
      this.setState({
        location: {
          latitude: position.coords.latitude,
          longitude: position.coords.longitude,
        },
        zoom: 12, // on zoom sur la location de l'utilisateur
        userHasALocation: true,
      });
    }, () => {
      // si l'utilisateur n'a pas accepté de nous fournir sa location
      fetch('https://ipapi.co/json') // on récupère la position en fonction de l'adresse IP de l'utilisateur
        .then(response => response.json())
        .then((position) => {
          this.setState({
            location: {
              latitude: position.latitude,
              longitude: position.longitude,
            },
            zoom: 12, // on zoom sur la location de l'utilisateur
            userHasALocation: false,
          });
        });
    });
  }

  loadProducer() {
    const { listProducers } = this.props;
    return (
      listProducers.map(tile => (

        <Marker position={[tile.position.lat, tile.position.lng]} icon={myIcon}>
          <Popup>
            A pretty CSS3 popup.
          </Popup>
        </Marker>
      ))
    );
  }

  render() {
    const { location, zoom, userHasALocation } = this.state;
    const { latitude, longitude } = location;

    return (
      <div className="mainPage">
        <Map className="map" center={[latitude, longitude]} zoom={zoom} ref={(c) => { this.map = c; }}>

          <TileLayer
            attribution="&amp;copy <a href=&quot;http://osm.org/copyright&quot;>OpenStreetMap</a> contributors"
            url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
          />

          {
            // si l'utilisateur a accepté de donner sa location, l'affiche sur la carte
            userHasALocation
            && (
              <CircleMarker center={[latitude, longitude]} />
            )

          }

          {
            this.loadProducer()
          }
        </Map>

      </div>
    );
  }
}

export default MyMap;
